    ...
    var body: some View {
        NavigationView {
            ZStack() {
                ...
                .fullScreenCover(isPresented: $navigateToAddTask1Week) {
                ...
                ZStack() {
                    VStack(alignment: .center, spacing: 0) {
                        ...
                        Rectangle()
                            ...
                            .frame(maxWidth: 140)  // 最大幅を300ptに設定
                            .offset(x:120, y: 1)
                    }.offset(x: -151)
                    .background(
                        // 実際の遷移を実行するNavigationLink
                        NavigationLink("",
                            destination: Addtask1dayView() // 遷移先のビューを指定
                                .navigationBarBackButtonHidden(true), // バックボタンを非表示にする
                            isActive: $navigateToAddTask1Day // 遷移の状態バインディング
                        )
                    )
...
struct Addtask1weekView_Previews: PreviewProvider {
